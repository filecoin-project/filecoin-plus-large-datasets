...
- If a client wants to onboard more than 5+PiBs, the recommendation would be to start with a few applications and earn trust from the community. Having a positive reputation and proving to the community first by onboarding a smaller amount of data will help anyone who wants to onboard massive amounts of data much faster and smoother. If clients are creating multiple applications, the easiest solution would be to assign different client addresses for every application. However, if this is not feasible, know that the workaround requires manual attention. We strongly advise against this workaround due to the potential delays created and mixed math for subsequent allocation issues. Although we can support this in the short term, please notify Simon Kim and add this to your LDN application if you absolutely have to go down this path and share why.
